...
if(isset($_POST['submit'])){
  ...
  $payment_id=$_POST['payment_id'];
    ...
    if(empty($payment_id) || empty($amount_paid)){
            echo "Don't leave any empty field"; 
        }


    if($amount_paid != $price) {  
            $Err = 'Amounts did not match';  
            } 
        ...
        if(!$conn){
           ...
           $sql3="DELETE from roombook where booking_number=$id";
            mysqli_query($conn,$sql3);

           $sql2="INSERT INTO payments(
            payment_id,fullname,room_id,price)
            VALUES('$payment_id','$fullname','$room_id','$price')";
...
<body>
  ...
  <div class="sidenav">        
      <a href="studentdashboard.php" class="acitve"><i class="fa fa-home" style="font-size:20px;"></i> HOME</a>
      <a href="addroom.php"><i class="fa fa-hotel" style="font-size:20px;"></i> BOOK ROOM</a>
      <a href="paymentdetail.php"><i class="fa fa-money" style="font-size:20px;"></i> PAYMENT DETAIL</a>
      <a href="managebookroom.php"><i class="fa fa-far fa-bell-slash" style="font-size:20px;"></i> BOOKED ROOM</a>
  ...
  <div class="col-75">
    ...
    <input type="text" name="fullname" value = "<?php echo $row_stud['fullname']?>">
     ...
     <input type="text" name="room_id" value="<?php echo $data1['room_id']?>" readonly="readonly">
             <label>Price</label> 
    <input type="text" name="room_id" value="<?php echo $select_bed * $price + $row2['totals'] ?>" readonly="readonly"></td>
    <br>
          </div>
    ...
    data_amount="$select_bed * $price + $row2['totals']"
    data-name="MCPS"
