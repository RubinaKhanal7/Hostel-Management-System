...
if(isset($_POST['submit'])){
    ...
    $price=$_POST['price'];
    $status=$_POST['status'];
    if(empty($id) || empty($no_of_bed) || empty($floor_number) || empty($price) ||   empty($status)){
    ...
    if($no_of_bed != 3) {
      $bedError = 'No. of bed should be 3';
        ...
        if(!$conn){
            ...
            $sql2="INSERT INTO room(
            room_id,no_of_beds,floor_number,price,status)
            VALUES('$id','$no_of_bed','$floor_number','$price','$status')";
      ...
      <form  method="post">
        <div class="row">
          <div class="col-50">
            ...
            <input type="text" name="no_of_bed" placeholder="" required value = <?php echo $_POST['no_of_bed']??''; ?>>
            <span class="error"><?php echo $bedError;?> </span><br>
            <label>FLOOR NUMBER</label>
            <input type="number" name="floor_number" placeholder="" required value = <?php echo $_POST['floor_number']??''; ?>><br>
             <label>PRICE</label>
            ...
            <div class="row">
              <div class="col-50">
                <input type="radio" name="status" class="radio" value="Available" required>AVAILABLE
              </div>
            </div>
          </div>
          
        </div>
         <input type="submit" name="submit" value="Add Room" class="btn" onclick="return confirm('Are you sure you want to add a room?')">
               </form>
    </div>
  </div>
 </div>
